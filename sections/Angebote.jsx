function Angebote({ acc }) {
  ...
  const items = [
    ...
    {
      n: '02', tail: 'Leadership Trainings', color: 'var(--twk-sage)', icon: 'group',
    ...
    {
      n: '03', tail: 'Leadership Bootcamp', color: 'var(--twk-terracotta)', icon: 'glove',
      claim: 'Im Ring. In der Verantwortung.',
      d: 'Intensiv und fordernd: Führung üben, wo es knirscht — und Verantwortung real übernehmen.',
    },
  ];
  return (
    <section id="angebote" data-screen-label="Angebote" style={{ padding: '120px 40px', background: 'var(--neutral-50)' }}>
      <div style={{ maxWidth: 'var(--container-lg)', margin: '0 auto' }}>
        <div style={{ marginBottom: 44 }}>
          ...
          <p style={{ font: 'var(--text-lead)', color: 'var(--color-fg-muted)', maxWidth: 600, margin: '20px 0 0' }}>
            Drei Wege, ein Anspruch: Wirksamkeit statt nette Gespräche.
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }} className="grid-3">
          {items.map((s) => (
            <div key={s.tail} className="twk-svc-card" style={{ position: 'relative', display: 'flex', flexDirection: 'column', background: 'var(--neutral-900)', borderRadius: 'var(--radius-lg)', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)' }}>
              ...
              <button onClick={() => goTo('kontakt')} className="twk-svc-cta" style={{ border: 'none', cursor: 'pointer', background: s.color, color: '#fff', fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: 15, padding: '15px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
