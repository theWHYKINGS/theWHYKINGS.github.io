function FinalCTA({ acc }) {
  ...
  const expect = [
    '30–45 Minuten, ehrlich und ohne Verkaufsdruck.',
    'Eine klare Einschätzung, ob ich der Richtige bin.',
    'Ein Nein, wenn es ein Nein sein sollte.',
  ];
  return (
      ...
      <div style={{ position: 'relative', maxWidth: 'var(--container-md)', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 56, alignItems: 'start' }} className="finalcta-grid">
        <div>
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 'clamp(2.1rem,3.8vw,3rem)', color: '#fff', margin: '14px 0 18px', letterSpacing: '-0.02em', lineHeight: 1.08 }}>
            Wenn du Klarheit willst, sollten wir sprechen.
          </h2>
          <p style={{ font: 'var(--text-lead)', color: 'rgba(255,255,255,0.74)', maxWidth: 440, marginBottom: 30 }}>
            Ein erstes Gespräch ist kein Verkaufstermin. Wir klären, ob und wie eine Zusammenarbeit Sinn ergibt.
          </p>
          ...
          {sent ? (
            ...
            <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <label style={{ display: 'flex', flexDirection: 'column', gap: 6, fontFamily: 'var(--font-body)' }}>
                <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-fg)' }}>Worum geht's?</span>
                <textarea value={form.message} onChange={set('message')} rows={4} placeholder="Ein paar Zeilen zu deiner Situation unter Druck…"
                  style={{ fontFamily: 'var(--font-body)', fontSize: 15, padding: 12, borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border-strong)', resize: 'vertical', color: 'var(--color-fg)' }} />
              </label>
